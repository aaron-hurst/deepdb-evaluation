...
def create_naive_all_split_ensemble(
    schema,
    hdf_path,
    sample_size,
    ensemble_path,
    ...
    meta_data_path = hdf_path[:-4] + "_meta_data.pkl"
    ...
    for table_obj in schema.tables:
        ...
        if incremental_learning_rate > 0:
            (
                ...
                null_values,
                full_join_est,
            ) = prep.generate_n_samples_with_incremental_part(
                sample_size,
            ...
            logger.debug(
                f"Requested {sample_size} samples and got {len(df_samples)} + {len(df_inc_samples)} "
        ...
        min_instance_slice = RATIO_MIN_INSTANCE_SLICE * min(
            sample_size, len(df_samples)
        )
        ...
        if incremental_learning_rate > 0:
            logger.info(
                f"additional incremental SPN training phase with {len(df_inc_samples)} samples "
...
def naive_every_relationship_ensemble(
    schema,
    hdf_path,
    sample_size,
    ensemble_path,
    ...
    ensemble_path += (
        "/ensemble_relationships_" + dataset + "_" + str(sample_size) + ".pkl"
    )
    logger.info(f"Saving ensemble to {ensemble_path}")
    spn_ensemble.save(ensemble_path)
